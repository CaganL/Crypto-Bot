...
# Anahtarları topla (3 Anahtar Gücü)
API_KEYS = []
...
# --- 4. AI MOTORU (V17.0 ZEKASI + 3 ANAHTAR GÜCÜ) ---
async def get_ai_comment(symbol, price, rsi, direction, score, news_title, macro_low, macro_high, history_str):
    ...
    # --- ESKİ FAVORİ MODELLER GERİ DÖNDÜ ---
    # Sırayla en kaliteliden en hızlıya doğru dener
    models = [
        ("Gemini 1.5 Pro", "gemini-1.5-pro"),       # En Zeki (Efsane olan)
        ("Gemini 2.0 Flash", "gemini-2.0-flash"),   # En Yeni Hızlı
        ("Gemini 1.5 Flash", "gemini-1.5-flash")    # Yedek Güç
    ]

    last_error = ""

    # ÖNCE MODELİ SEÇ, SONRA ANAHTARLARI DENE
    for model_name, model_id in models:
        ...
        for api_key in API_KEYS:
            ...
            try:
                ...
                # Timeout'u 40 saniye yaptık (Pro model yavaş düşünür, sabırlı olalım)
                resp = await asyncio.to_thread(requests.post, url, headers=headers, json=payload, timeout=40)
                
                if resp.status_code == 200:
                    raw_text = resp.json()['candidates'][0]['content']['parts'][0]['text']
                    return clean_markdown(raw_text) + f"\n\n_(🧠 {model_name} | 🔑 {key_short})_"
                
                elif resp.status_code == 429:
                    # Kota dolduysa sessizce diğer anahtara geç
                    continue 
                else:
                    # Başka hata varsa da geç
                    continue
        ...
        # Bu model hiçbir anahtarla çalışmadıysa, döngü bir sonraki modele (Flash'a) düşer.

    return f"⚠️ Analiz alınamadı. Tüm modeller ve anahtarlar denendi.\nSon Hata: {last_error}"
...
async def incele(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    msg = await update.message.reply_text(f"🚀 *{symbol}* için V17.5 Legend çalışıyor...", parse_mode='Markdown')
...
if __name__ == '__main__':
    print("🚀 BOT V17.5 LEGEND ÇALIŞIYOR...")
    app = ApplicationBuilder().token(TELEGRAM_TOKEN).build()
    app.add_handler(CommandHandler("incele", incele))
    # Conflict hatasını önlemek için temizlik komutu:
